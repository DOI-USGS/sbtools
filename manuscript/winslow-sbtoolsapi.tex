...
with the \code{item_get} function.
...
% do you need to call item_get(test_item)$citation, or can you just use
% test_item$citation?
...
% feel free to improve
 ...
 \end{figure}
% should figure out & say what the map means and include citation
...
functionality.
% i want more specifics in the previous sentence. what constitutes 'basic'?
% what non-text features are available? when you search text, which text
% are you searching? what's so low-level about searching the metadata? when
% you search the metadata, are you searching stuff like timestamps or like
% the paragraphs of metadata text that an author put in there?
...
% need a free text query example
The free text of a ScienceBase item includes the item's title, authors, ....
This text can be searched with the \code{query_???} function.
...
% it would be great if query_sb_doi could search for dois with or without a
% preceding 'doi:' without the user ever having to include that 'doi:' text
...
\begin{example}
#some item DOIs have 'doi:' appended to the front of them for some reason
...
% do they need to be loaded, as well?
...
% can we get away with keeping query_items to ourselves and just talking about
% query_sb here? Seems very low level and useful to only a small subset of users.
...
% these are available above, aren't they?
...
To create new items, the only required input to \code{item_create} is "title".
...
\end{example}
% is it possible to query for just one or two of these, e.g., just scheme & type?
